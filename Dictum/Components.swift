...
#Preview("CustomTextEditor") {
    struct PreviewWrapper: View {
        @State private var text = "Hello World! Привет мир!"

        var body: some View {
            CustomTextEditor(text: $text, onSubmit: {})
...
#Preview("GreenToggleStyle") {
    VStack(spacing: 16) {
        Toggle("Включено", isOn: .constant(true))
            .toggleStyle(GreenToggleStyle())
        Toggle("Выключено", isOn: .constant(false))
            .toggleStyle(GreenToggleStyle())
    }
    .padding()
    .background(Color.black)
...
#Preview("SettingsSection") {
    SettingsSection(title: "НАСТРОЙКИ") {
        SettingsRow(title: "Опция 1", subtitle: "Описание опции") {
            Toggle("", isOn: .constant(true))
                .toggleStyle(GreenToggleStyle())
        }
        SettingsRow(title: "Опция 2") {
            Text("Значение")
                .foregroundColor(.gray)
        }
    }
...
#Preview("HotkeyRow") {
    VStack(spacing: 8) {
        HotkeyRow(action: "Открыть", keys: ["⌘", "+", "§"], note: nil)
        HotkeyRow(action: "Закрыть", keys: ["Esc"], note: "⚠️ Не работает")
    }
    .padding()
    .background(Color.black)
}
