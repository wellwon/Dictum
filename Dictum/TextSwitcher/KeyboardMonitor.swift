...
//
//  ⛔ РАЗРЕШЕНИЯ: Проверяем ТОЛЬКО AXIsProcessTrusted() (Accessibility)!
//  Input Monitoring НЕ НУЖЕН — Accessibility покрывает CGEventTap .listenOnly
//  НЕ добавлять CGPreflightListenEventAccess() или другие проверки Input Monitoring!
...
import ApplicationServices  // Для AXIsProcessTrusted() — ЕДИНСТВЕННАЯ проверка разрешений!
import os
